...
#     #     cursor_focus = self.student_table.focus()
#     #     content = self.student_table.item(cursor_focus)
#     #     if len(content["values"]) > 0:  # Kiểm tra xem có dữ liệu nào không trước khi truy cập
#     #         data = content["values"]
#     #         self.var_dep.set(data[0])
#     #         self.var_course.set(data[1])
#     #         self.var_year.set(data[2])
...
class Student:
    ...
    #                     self.var_teacher.get(),
    #                     self.var_radio1.get(),
    #                     self.var_std_id.get()  # Fixed this line
    #                 ))
    #             conn.commit()
    #             self.fetch_data()
    #             self.reset_data()
    ...
    #                     face = cv2.cvtColor(face, cv2.COLOR_BGR2GRAY)
    #                     # file_name_path = "Data/User." + str(id) + "." + str(img_id) + ".jpg"
    #                     file_name_path = "Data/User." + str(studen_id) + "." + str(img_id) + ".jpg"
    #
    #                     cv2.imwrite(file_name_path, face)
    #                     text_position = (10, 30)
    #                     cv2.putText(face, str(img_id), text_position, cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 2)
